...
Tarkoitus asentaa Wazuh palvelinkomponentit ja Wazuh Agent palvelimeen Saltin avulla käyttäen Vagrant virtuaalikoneita. 

Wazuh palvelinkomponentteihin kuuluu paketit Wazuh Indexer (vastaa datan indeksoinnista päätelaitteilta), Wazuh Manager (vastaa datan analysoinnista, tietoturvan hallinnasta ja valvonnasta) ja Wazuh Dashboard (vastaa tiedon visualisoinnista).
...
Aloitin tilojen luomisen `server.sls` tilalla, joka asentaisi palvelinlaitteelle Wazuh palvelinkomponentit.
...
Tiedosto...
* Määrittää server_ip muuttujan (tämä on manuaalisesti vaihdettava oikeaksi ympäristöstä riippuen).
* Varmistaa, että Wazuh repository on lisättynä.
...
Lisätty Agentti "agent" vastaa yhdistämäni virtuaalikoneen tietoihin. Agenttia klikkaamalla sain näkymän "Endpoints > agent"-näkymän esille.

![image](https://github.com/user-attachments/assets/73dde8d7-c6e4-43c4-8f6a-c406aeac67ce)

Näkymästä nähdään, että "agent" -laitteen konfiguraatiosta on muodostettu skannauksen perusteella haavoittuvuusilmoituksia, joka tarkoittaa, että Wazuh Indexer on pystynyt keräämään tiedot agentilta. Kokonaisuus, siis toimii haluamallani tavalla.
...
Jotta molemmat tilat varmasti toimivat, niin testasin vielä ajaa ne kummatkin verkon yli minioneilla. Kummankin tilan tulisi tukea Ubuntun lisäksi Debiania, joten käytin tällä kertaa Debian koneita.

Tallensin .sls-tiedostot GitHubiin ja tuhosin edelliset virtuaalikoneet.

Loin vagrantilla kolmen debian bookworm virtuaalikoneen verkon kuvan Vagrantfilellä. 
...
Tilan ajo epäonnistui, joka johtui siitä, että Debian ei tue Saltin key_url ominaisuutta. 

Testeistä sain selville, että tila `server.sls` toimii ainakin Debianissa ja Ubuntussa, ja `agent.sls` vain Ubuntussa.

Jos tulevaisuudessa moduulia haluaisi parantaa, niin `àgent.sls` tilasta voisi tehdä Debian yhteensopivan.
